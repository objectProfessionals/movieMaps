 ...
 * 
 */
public class ScriptPathMinimal {
	public static ScriptPathMinimal tester;
	private boolean large = false;
	private boolean bordered = false;
	private boolean saveFinal = true;
	private boolean toFrame = false;
	private boolean scaled = false;
	protected String scName = "BR";
	...
	protected double borderWF = 0.01;
	protected double bHeightF = 0.05;
	protected double borderHF = 0.05;
	protected double ar = 1.4142857142857142857142857142857;
	...
	double fontScFScenes1 = 0.11; // 0.09;
	...
	protected static final int ALIGN_45_RIGHT_DOWN = 8;
	...
	private float sceneBlockColAlpha = 0.15f;
	private float sceneTextColAlpha = 1f; // 0.25f;
	private Color sceneTextColor = Color.BLACK;
	...
	protected void plotRecords() throws FontFormatException, IOException {
		...
		int hh = height + (2 * bHeight) + (borderH);
	...
	private void setupSceneColors() throws IOException {
		String src = SharedUtils.HOST_MODULE + SharedUtils.DIR_PROCESS + SharedUtils.DIR_IMAGES + SharedUtils.DIR_OUT
				+ dir;
		File inputFile = new File(src + scName + paletteFile);
		bgImage = ImageIO.read(inputFile);
	...
	protected void plotBlankBackground() throws IOException {
	...
	protected void plotSceneNames() {
		...
		for (Scene sc : scenes) {
			...
			int sceneWidth = (int) (x2 - x1);
			int sceneBorders = 5;
			Shape scclip = new Rectangle((int) (borderW + bWidth + x1 + sceneBorders), sceneBorders,
					sceneWidth - sceneBorders, height - sceneBorders + (2 * (borderH + bHeight)));

			BufferedImage bi = new BufferedImage(10, 10, BufferedImage.TYPE_INT_ARGB);
			Graphics2D big = bi.createGraphics();
			big.setColor(getColorWithAlpha(sc.col, sceneBlockColAlpha));
			Stroke ns = new BasicStroke(4);
			big.setStroke(ns);
			if (flip) {
				// big.drawLine(0, 5, 5, 0);
				big.drawLine(-2, 12, 12, -2);
				// big.drawLine(5, 10, 10, 5);
			} else {
				// big.drawLine(5, 0, 10, 5);
				big.drawLine(-2, -2, 12, 12);
				// big.drawLine(0, 5, 5, 10);
			}
			flip = !flip;
			Rectangle r = new Rectangle(0, 0, 10, 10);
			opG.setPaint(new TexturePaint(bi, r));

			opG.fill(scclip);
			plotSceneNameText(sc, x1, x2);
		}
		System.out.println("scenes...");
	}

	private void plotSceneNameText(Scene sc, double x1, double x2) {
		int sceneWidth = (int) (x2 - x1);
		boolean spinSceneNames = false;
		int num = 1;

		opG.setColor(getColorWithAlpha(sc.col, sceneTextColAlpha));
		...
		if (!"".equals(tot.trim()) && x1 != x2) {
			...
			if (spinSceneNames && l >= height) {
				...
				int xoff = (int) (allFontMult * fontScFScenes * 20 * 7) + sceneWidth / 2;
				int yoff = (int) (allFontMult * fontScFScenes * 20 * 10);
				opG.setClip(null);
				plotString((int) x1 + xoff, tot, borderH + bHeight - yoff, ALIGN_45_RIGHT_DOWN,
						allFontMult * fontScFScenes * (20), false, sceneFontLowerCase, false);
			}
		}
	}
	...
	protected int plotString(double x1, String name, double y, int align, double scale, boolean outline,
		...
		if (align == ALIGN_V_RIGHT) {
			...
			at.concatenate(tr2);
		} else if (align == ALIGN_45_RIGHT_DOWN) {
			AffineTransform tr2 = AffineTransform.getTranslateInstance(
					-((double) glyph.getBounds2D().getWidth()) * scale * 0.707,
					-((double) glyph.getBounds2D().getWidth()) * scale * 0.707);
			at.concatenate(tr2);
		}
		at.concatenate(tr);
		if (align == ALIGN_V_LEFT || align == ALIGN_V_RIGHT || align == ALIGN_V_CEN_Y) {
			...
			AffineTransform ro = AffineTransform.getRotateInstance(Math.PI / 4);
			at.concatenate(ro);
		}
		at.concatenate(sc);
		Shape transformedGlyph = at.createTransformedShape(glyph);
	...
	protected void plotTitle() throws FontFormatException, IOException {
		...
		plotString((int) (-bWidth) + characterD * 0.25, nt, (height), ALIGN_V_RIGHT, characterD * 0.1, titleOutline,
