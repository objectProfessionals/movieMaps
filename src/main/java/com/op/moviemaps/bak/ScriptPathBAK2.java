...
public class ScriptPathBAK2 {
	public static ScriptPathBAK2 tester;
	protected static String scName = "FOTR";
	...
	private String paletteFile = "Paint.png";
	private String volFile = "Paint.png";
	boolean locationFontOutline = false;
	boolean sceneFontLowerCase = true;
	boolean locationFontLowerCase = true;
	boolean charFontLowerCase = true;
	boolean titleFontLowerCase = false;
	double charFontOffsetF = 0.4;

	public static void main(String[] args) throws IOException,
			FontFormatException {
		tester = new ScriptPathBAK2();
		tester.createImage();
	}
	...
	private void drawBGFromGradient() {
		GradientPaint gp = new GradientPaint(0, height / 2
				+ (bHeight + borderH), bgCol1, width + (bWidth + borderW) * 2,
				height / 2 + (bHeight + borderH), bgCol2, true);
		Paint bef = opG.getPaint();
		opG.setPaint(gp);
		opG.fillRect(0, 0, width + (bWidth + borderW) * 2, height
				+ (bHeight + borderH) * 2);
		opG.setPaint(bef);
	}
		...
		for (Scene sc : scenes) {
			...
			if (!"".equals(tot.trim()) && x1 != x2) {
				plotString((int) x1, tot, borderH + bHeight, ALIGN_V_LEFT,
						fontScFScenes * (x2 - x1), false, sceneFontLowerCase);
			}
		}
		opG.setClip(null);
		...
		for (Character ch : chs) {
			...
			if (!ch.points.isEmpty()) {
				...
				if ((int) x11 == (int) prev.x2) {
					...
					ch.addPoint(prev.x2, prev.y2, x11, y1, scenes.indexOf(sc),
							true);
				}
			}
	...
	protected int plotString(double x1, String name, double y, int align,
			double scale, boolean outline, boolean lowerCase) {
		if (lowerCase) {
	...
	private void createPath(Character ch, double xOff, double yOff) {
		...
		plotString(pFirst.x1 - xOffset + xFontOffset, name, borderH + bHeight,
				ALIGN_V_LEFT, fontSc, false, charFontLowerCase);
		if (ch.death.x1 != 0) {
			plotString(pe.x1 - xOffset + xFontOffset, name, height,
					ALIGN_V_RIGHT, fontSc, false, charFontLowerCase);
		} else {
			plotString(pLast.x2, getRealName(ch.description), pLast.y2
					+ xOffset - xFontOffset + bHeight + borderH, ALIGN_H_RIGHT,
	...
	protected void saveActualImage() throws IOException {
		...
		opG.dispose();
		System.out.println("saved " + fFile1.getName());
	}
	...
	protected void saveImageClipped() throws IOException {
		...
		opG.dispose();
		System.out.println("save clipped " + fFile1.getName());
	}
	...
	protected void plotLocationNames() {
		for (Location loc : locations) {
			...
			plotString((int) (-bWidth), tot, y, ALIGN_H_LEFT_CEN_Y,
					fontScFLocNames * h, locationFontOutline,
					locationFontLowerCase);
			opG.setClip(null);
		}
	...
	protected void plotTitle() {
		...
		plotString(-bWidth - borderW + bWidth * 0.01, nt,
				(borderH + bHeight + height * 0.5), ALIGN_V_CEN_Y,
				bWidth * 0.01, titleOutline, titleFontLowerCase);
